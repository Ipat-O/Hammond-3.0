...
## D-025 — Retain current app; stop feature development and publish main

Owner clarified that Hammond is a manual project workflow tool: inject instructions, create/organize tasks, add comments and monitor progress. Keep current Supabase persistence and local directory integration. Work orders/results may be ordinary manually entered task comments, not a new automation subsystem. MCP/Docker, OAuth work and disk-only replacement are cancelled; D-025 supersedes D-022/D-023/D-024. HAM3-014 and unexecuted HAM3-012 release expansion are cancelled. Only specifically requested UI improvements may be considered later.

Owner authorized publishing the relevant accepted app to main, preparing Claude's machine residue scoping job and Luna's detailed user-guide work order. No main branch previously existed; establish it from the accepted dev app plus truthful documentation and cancellation records. Retain source, migrations, tests and historical tracker records; never import cancelled PR #12 code or local credentials/artifacts. No owner database/settings changes or installer release claim. Existing workflow's dev integration convention is superseded only for this owner-authorized main baseline publication.

Cleanup is inventory first, with exact paths, ownership/state evidence and proposed removal batches. No blanket machine deletion authorization is inferred from scoping. Luna's work is documentation only on the pinned main baseline; no feature work. These non-feature jobs may be prepared together because one is read-only machine scoping and the other is isolated documentation. Owner transfers both local work orders; no agent execution is claimed.
