...
## 2026-09-09 — HAM3-014 added as a design document

- Owner requested an implementation plan on 2026-09-09 for LLM/MCP agent access to Hammond project and task memory and to instruction scopes, explicitly including instruction scope in the request. This is a new task, not a revival of the cancelled HAM3-009 work-order UI or the cancelled HAM3-010/HAM3-011 SHA-approval and search work.
- HAM3-014 created in `in_design`, depending on the already-merged HAM3-004, HAM3-005, HAM3-006, HAM3-007, HAM3-008, and HAM3-013. The foundation plan extends from thirteen tasks to fourteen.
- Proposed first-version architecture: a packaged local MCP stdio companion talks to the running Hammond desktop app over an authenticated Windows named pipe, through a narrow typed command facade over the existing instruction services and data repositories. No HTTP listener, no separately installed runtime, and no second owner login. This is recorded as a proposed narrow exception to D-001's no-bridge wording; the task file defers settling that boundary to dispatch review rather than deciding it now.
- The task file fixes a nine-tool typed contract (`get_project_context`, `list_tasks`, `get_task`, `get_instructions`, `list_instruction_versions`, `get_instruction_version`, `create_task`, `update_task`, `add_comment`), a required project/role/provider instruction-scope contract reusing the existing composition service rather than a parallel MCP-only composer, and shared desktop/MCP task-write invariants: monotonic task revisions, durable request-ID deduplication, and hierarchy/archive race handling between the UI and MCP callers.
- Four bounded implementation checkpoints are recorded inside the one task rather than as separate dispatched tasks: connection and vertical read, read context and instruction scopes, task writes and reconciliation, and setup/delivery. The task explicitly states writes must not be exposed until the revision/deduplication/race invariants pass.
- Per the task's own dispatch requirements, HAM3-014 stays `in_design` until the orchestrator verifies merged dependencies (already true), settles the proposed bundled-IPC boundary, and prepares an exact-base worker packet with an actual branch and worker identity. No task promoted, no worker dispatched, no start SHA invented, and no branch created beyond this planning commit. HAM3-014 is recorded to be planned ahead of the HAM3-012 integrated release, since HAM3-012 is expected to include this feature's packaging and smoke after delivery.
- `tracker/index.yaml` and `tracker/BOARD.md` updated so no canonical document is missing the new task; the board's stale "Dependency waves" list (which had not been updated when HAM3-013 was added ad hoc) now also records HAM3-013 and HAM3-014 as dependency-satisfied ad hoc additions rather than being silently left further out of date.
